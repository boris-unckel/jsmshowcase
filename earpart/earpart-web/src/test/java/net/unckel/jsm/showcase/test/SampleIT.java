package net.unckel.jsm.showcase.test;
...
public class SampleIT {
    ...
    public static Archive<?> getEarArchive() {
        ...
        EnterpriseArchive ear = ShrinkWrap.create(EnterpriseArchive.class, "earpart-ear.ear");

        // Current directory is the root of the "earpart-web" project. Go up one level
        // and enter the "earpart-ejb" project.
        // The ejb jar is found in the "target" directory:
        File f = new File("../earpart-ejb/target/earpart-ejb.jar");
        JavaArchive ejbJar = ShrinkWrap.create(ZipImporter.class, "earpart-ejb.jar").importFrom(f).as(JavaArchive.class);
        ear.addAsModule(ejbJar);

        // Now grab the web archive:
        f = new File("./target/earpart-web.war");
        if (f.exists() == false) {
            throw new RuntimeException("File " + f.getAbsolutePath() + " does not exist.");
        }
        WebArchive war = ShrinkWrap.create(ZipImporter.class, "earpart-web.war").importFrom(f).as(WebArchive.class);
        ear.addAsModule(war);
        ...
        war.addPackage("net.unckel.jsm.showcase.test");
